...
public class Bear extends Predator {
    ...
    public synchronized void reproduction() {
        if ((cell.arrayList.stream().filter(s ->s instanceof Bear).count()>2)&&
                (cell.arrayList.stream().filter(s->s instanceof Bear).count() < MAX_QUANTITY_CELL.MAX_Bear.getQUANTITY())) {
            cell.arrayList.add(new Bear(cell));
        }
    }
    ...
    public synchronized void move() {
            for (int i = 0; i < cell.arrayList.size(); i++) {
                if (cell.arrayList.get(i) instanceof Bear){
                    cell.arrayList.remove(cell.arrayList.remove(i));
                    break;
                }
            }
            while (true) {
            int a = ThreadLocalRandom.current().nextInt(MAX_MOVE_ANIMAL.MAX_Bear.getMAX_MOVE());
            int b =  ThreadLocalRandom.current().nextInt(MAX_MOVE_ANIMAL.MAX_Bear.getMAX_MOVE());
            if (MapOfCells.cells[a][b].arrayList.stream().filter(s -> s instanceof Bear).count() < MAX_QUANTITY_CELL.MAX_Bear.getQUANTITY()){
                MapOfCells.cells[a][b].arrayList.add(new Bear(MapOfCells.cells[a][b]));
            break;
            }
            }
    ...
    public synchronized void eat() {
        ...
        if (saturationOfDay <= SATURATION_ANIMAL.MAX_Bear.getSATURATION()){
            move();
        }
    ...
    public synchronized int hunt() {
        int whoseHunt = ThreadLocalRandom.current().nextInt(10);
        ...
        switch (whoseHunt) {
            ...
            case 1:{
                if (successHunt < 80 ) {
                    ...
                    return (int) WEIGHT_ANIMAL.MAX_Snake.getWEIGHT();
                }
            }
            case 2: {
                if (successHunt < 40 ) {
